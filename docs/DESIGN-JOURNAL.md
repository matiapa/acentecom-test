...
Final verified state (against the live store): 17 products, 26 variants, 5 paid orders, 7 line items sync idempotently (re-run leaves counts flat); the report reconciles to `$8450.34`; the `store-analyst` agent, live over the read-only MCP, answered "5 orders / $8450.34 / top product The Complete Snowboard (9 units)" — every number matching a direct SQL cross-check, each citing its query.

## 12. The agent audited its own database

The most satisfying moment: on the live Q&A run, the agent's `list_tables` call returned a **critical security advisor** — RLS was disabled on all seven public tables. Supabase grants the PostgREST roles (`anon`/`authenticated`, reachable with the publishable key) broad access by default and relies on RLS to restrict it; with RLS off, the whole store was readable over the auto REST API. The agent flagged it, gave the remediation SQL, and correctly **did not apply it** (it's read-only, and that's the owner's call).

The fix had one trap worth understanding: the naive move — `revoke ... from anon` — could have cut off the agent too, if it read via those roles. It doesn't: `supabase_read_only_user` reads via `pg_read_all_data` membership and its own `BYPASSRLS` attribute, entirely independent of `anon`/`authenticated`. So enabling RLS (deny-all) blocks the REST roles while the agent (bypass) and the ETL (owner `postgres`) are untouched. Views needed `security_invoker = true` as well, or `anon` could still read *through* them. Verified after applying: `anon` hard-denied on tables and views; the agent returned the identical numbers. My earlier "grant SELECT to anon/authenticated" in `0002` turned out to be both unnecessary and the thing that widened the surface — removed.

Lesson for the interview: a read-only analytics agent isn't just a feature, it's a **standing auditor** of the data layer — and least-privilege only works if you know *which* privilege path each actor actually uses before you revoke anything.
